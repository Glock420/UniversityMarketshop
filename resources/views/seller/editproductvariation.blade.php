            ...
            <ul>
                @foreach ($colors as $color)
                    @if ($color->color !== null)
                                ...
                                <form action="{{ route('remove.color') }}" method="POST">
                                    @csrf
                                    <div class="row removeRow">
                                        <div class="col-md-6 d-flex align-items-center">
                                            {{ $color->color }}
                                            <input type="hidden" name="prod_id" value="{{ $productdetails->prod_id }}">
                    ...
                    @endif
                @endforeach
                <b><br> Add New Color:</b>
                <form action="{{ route('add.color') }}" method="POST">
                    @csrf
                    <input type="hidden" name="prod_id" value="{{ $productdetails->prod_id }}">
                    <input type="text" name="color">
                    <button type="submit" class="btn btn-danger">Add Color</button>
                </form>
            </ul>
            ...
            <ul>
                @foreach ($sizes as $size)
                    @if ($size->size !== null)
                        ...
                        <form action="{{ route('remove.size') }}" method="POST">
                            @csrf
                            <div class="row removeRow">
                                <div class="col-md-6 d-flex align-items-center">
                                    {{ $size->size }}
                ...
                <form action="{{ route('add.size') }}" method="POST">
                    ...
                    <button type="submit" class="btn btn-danger">Add Size</button>
                </form>
            </ul>
            <br>
            <a href="{{ url('/seller/product/editproduct/'.$productdetails->prod_id) }}" class="btn btn-dark" style="width: 50%;">Back</a><br>
        </div>
